    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                WHERE (d.is_test_file IS NULL OR d.is_test_file = 'N') AND d.project_id = %s""",
                (project_id, )
            )
...
def db_create_dataset(project_id, dataset_id, table_path, n_past_week_for_training, n1_next_week_for_prediction, n2_next_week_for_prediction, column_list, data):
    ...
    query = """
        INSERT INTO datasets(project_id, dataset_id, raw_data_table_path, n_past_week_for_training, n1_next_week_for_prediction, n2_next_week_for_prediction)
        VALUES (%s, %s, %s, %s, %s, %s)
    """
...
def db_update_dataset(project_id, dataset_id, target_table_path, base_data_table_path):
    query = """
        UPDATE datasets
        SET target_data_table_path = %s, base_data_table_path = %s
    ...
    cur.execute(query, (
        target_table_path, base_data_table_path, project_id, dataset_id,
    ))
...
def db_create_target_table(key_column, date_column, target_column, table_path, forecasting_horizon):
    query = f"""
        ...
        select s1.{key_column}, s1.{date_column}, s2.{date_column} as forecasting_date, SUM(s2.{target_column}) as forecasting_target_value
        from {table_path} s1
        join {table_path} s2 on s1.{key_column} = s2.{key_column}
        where s2.{date_column}::date - s1.{date_column}::date = {forecasting_horizon} * 7
        group by s1.index_key, s1.cutoff_date, s2.cutoff_date;
    """
...
def db_create_base_data_table(raw_data_table_path, target_data_table_path, base_data_table_path, key_column, date_column):

    query = f"""
        drop table if exists {base_data_table_path};

        create table {base_data_table_path} as 
        select s1.{key_column}, s1.{date_column}, s2.forecasting_date, SUM(s2.forecasting_target_value) as forecasting_target_value
        from {raw_data_table_path} s1
        join {target_data_table_path} s2 on s1.{key_column} = s2.{key_column} and s1.{date_column} = s2.{date_column}
        group by s1.{key_column}, s1.{date_column}, s2.forecasting_date;
    """
