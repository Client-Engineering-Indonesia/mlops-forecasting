...
def get_datasets_by_projectid(project_id: str):
    ...
    datasets = []
    for table_path in result_df["base_data_table_path"].unique():
        dataset = result_df[result_df["base_data_table_path"] == table_path]

        dataset_info = dataset[["project_id", "dataset_id", "base_data_table_path", "target_data_table_path", "n_past_week_for_training", "n1_next_week_for_prediction", "n2_next_week_for_prediction"]]
        ...
        dataset_info["columns"] = [val for idx, val in dataset[["column_name", "column_type", "is_forecast_key", "is_target", "is_date", "is_feature"]].iterrows()]
...
def update_dataset(payload: UpdateDatasetSchema):
    ...
    table_path = db_create_target_table(key_column, date_column, target_column, dataset["table_path"], dataset["n1_next_week_for_prediction"])
    db_update_dataset(
        payload.project_id, 
        payload.dataset_id,
        table_path,
