...
def get_table_sample_data(payload: SampleDataSchema):
    ...
    sample_data = db_execute_sql("SELECT " + ", ".join([col for col in columns_used]) + " FROM " + dataset["base_data_table_path"] + " LIMIT 5")
...
def create_basic_feature_store(payload: SampleDataSchema):
    ...
    llm_response2 = multiprocess_llm_answer({
        "input_prompt": f"Get sample of sql statement. Then, design feature store based on that sql sample using source table as below:\n{dataset['base_data_table_path']}",
    ...
    # Step 3: Design feature store
    llm_response3 = multiprocess_llm_answer({
        "input_prompt": f"Ensure your sql statement also store the result into table named {table_name}. If not you have to recreate sql statement.",
        "thread_id": llm_response1["thread_id"],
    })
    ...
    llm_response4 = multiprocess_llm_answer({
        "input_prompt": f"Execute sql statement. Use destination table name: {table_name}, project id: {dataset['project_id']}, and dataset id: {payload.dataset_id}",
        "thread_id": llm_response1["thread_id"],
    })
    ...
    return {
        "status": "success", 
        "llm_response": [llm_response1, llm_response2, llm_response3, llm_response4]
    }
